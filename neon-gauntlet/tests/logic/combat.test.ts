...
import { activeHitbox, applyDamage, attackBounds, canHit, hitboxesOverlap, hurtBounds, sameLane, type FighterState } from '../../src/game/systems/CombatSystem'
...
const punch: AttackDefinition = {
  ...
  hitbox: {
    ...
    frames: [
      ...
      { frame: 2, forwardOffset: 14, width: 42, targetHalfWidth: 14, laneRange: 0.095 },
    ],
  },
  durationMs: 230,
  activeAfterMs: 40,
...
describe('CombatSystem helpers', () => {
  ...
  it('builds facing-aware attack bounds from hitbox data', () => {
    expect(attackBounds(fighter({ attackFrame: 1 }), punch)).toEqual({ left: 110, right: 148 })
    expect(attackBounds(fighter({ attackFrame: 1, face: -1 }), punch)).toEqual({ left: 52, right: 90 })
    expect(hurtBounds(fighter({ x: 132 }), punch)).toEqual({ left: 118, right: 146 })
  })

  it('selects hitbox metadata from the visible action frame', () => {
    expect(activeHitbox(fighter({ attackFrame: 0 }), punch)?.width).toBe(22)
    expect(activeHitbox(fighter({ attackFrame: 2 }), punch)?.width).toBe(42)
  })

  it('overlaps hitboxes only when the visible target body intersects the active box', () => {
    expect(hitboxesOverlap(fighter({ attackFrame: 1 }), fighter({ x: 132 }), punch)).toBe(true)
    expect(hitboxesOverlap(fighter({ attackFrame: 1 }), fighter({ x: 163 }), punch)).toBe(false)
    expect(hitboxesOverlap(fighter({ attackFrame: 1, face: -1 }), fighter({ x: 68 }), punch)).toBe(true)
    expect(hitboxesOverlap(fighter({ attackFrame: 1, face: -1 }), fighter({ x: 132 }), punch)).toBe(false)
  })
